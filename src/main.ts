...
export class BacklogItemHierarchyWidget {
         ...
         public LoadWidget(widgetSettings, isReload) {
           $("#witable").hide();
           let customSettings = JSON.parse(widgetSettings.customSettings.data) as ISettings | null;
           ...
           if (customSettings !== null) {
             ...
             this.getWiTypes()
               ...
               .then(() => {
                 this.GetWIHierarchy([customSettings.wiId], customSettings).then(
                   e => {
                     // composition du tableau final
                     let result = this.ComposeTabTree(
                       customSettings.parentwiasheader
                     );
         ...
         private LoadTreeTableJquery(isReload: any, customSettings: ISettings) {
           $("#witable").treetable({ expandable: isReload ? false : customSettings.expandtree, clickableNodeNames: false, onInitialized: function() {
                 $("#witable tr").each((i, el) => {
                   let spaceExpand = 0;
                   if (customSettings.expandtree) {
                     spaceExpand = 20;
                   }
                   let nameWidth = 250 - (12 + $(el)
                         .find("td:nth-child(2) .indenter")
                         .outerWidth() + spaceExpand);
                   $(el)
                 ...
                 $("#witable #linkwi").on("click", event => {
                   ...
                   WorkItemServices.WorkItemFormNavigationService.getService().then(
                     service => {
                       service.openWorkItem(
                         Number(current.attr("data-tt-id")),
                         false
                       ); // open the wi dialog
                     }
                   );
         ...
         private WriteTableBody(result: any, customSettings: ISettings) {
           result.forEach(element => {
             ...
             let nameContent = `<img src="${type[0].icon.url}" class=\"imgwitype\" /> ${this.getNameColumnContent(element.name, element.id, isHeader)}`;
             if (isHeader) {
               this.$title.html(nameContent);
             } else {
               let stateContent = this.GetColorColumnContent(element.state) + " " + element.state;
               this.appendTableColumn(this.Tablecontainer, [element.id, nameContent, stateContent], element.id, element.parentid);
             }
           });
         }
